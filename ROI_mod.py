...
def plot_RFs(rois, number=None, f_w =None,cmap='inferno',
    ...
    for idx, roi in enumerate(rois):
        ...
        center_RF = copy.deepcopy(roi.RF_map_norm)
        center_RF[center_RF<center_val] =np.nan
        sns.heatmap(roi.RF_map_norm, cmap=cmap, ax=ax[idx], cbar=False)
        if center_plot:
            sns.heatmap(center_RF,ax=ax[idx], cbar=False,alpha=.5,
                        cmap='Greens')
        ax[idx].axis('off')
    ...
    for roi in rois:
        ...
        for idx,  take in enumerate(req_epochs):
            if take:
                ...
                baseline = \
                    curr_trace[int(roi.imaging_info['frame_rate']/2):int(roi.imaging_info['frame_rate'])].mean()
...
def analyze_A_B_step(rois,int_rate = 10):
    ...
    for roi in rois:
        ...
        for epoch in roi.whole_trace_all_epochs:
            ...
            ms_500 = int(0.5 * roi.imaging_info['frame_rate'])
            
            roi.a_step_baseline_responses[epoch] = \
                np.nanmean(A_t[-ms_500:]) - base_activity
            roi.b_step_baseline_responses[epoch] = \
                np.nanmean(B_t[-ms_500:]) - base_activity
                
            roi.a_to_b_step_responses[epoch] = np.nanmax(B_t[:ms_500]) - np.nanmean(A_t[-ms_500:])
...
def analyze_luminance_gratings(rois):
    ...
    for roi in rois:
        ...
        roi_dict['TF'] = roi.stim_info['epoch_frequency'][1:]
