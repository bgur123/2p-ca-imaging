...
@author: burakgur/catay aydin / Functions for 2 photon image and data 
    processing. Mostfunctions are created by Catay Aydin.
"""
...
def motionCorrection(baseName, outDir, rawDataFile, granularity ='plane',
    ...
    match = [int(re.search(r'Cycle(.*?)_',
                                  imageName).group(1)) for imageName in allImages]
    ...
    if exportFrames:
        ...
        dataset.export_frames([[[motCorrDir + '/' + 'motCorr.tif']]],
                              fill_gaps=True)
    print(">>Motion correction done with %s..." % baseName)

    return motCorrDir, nonAlignedDatasetDir
