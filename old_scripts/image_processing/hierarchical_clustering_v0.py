...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 28 17:15:05 2019
...
#%% use pixel coordinates and time information 
DSI_metric = np.transpose(DSI_image.flatten())
SNR_metric = np.transpose(SNR_max_matrix.flatten())
CSI_metric = np.transpose(CSI_image.flatten())
X_coord = np.repeat(np.arange(mov_xDim),mov_yDim)
Y_coord = np.transpose(np.repeat(np.arange(mov_yDim),mov_xDim))
cluster_metrics = np.transpose([DSI_metric, SNR_metric, CSI_metric,X_coord,Y_coord])

#%%
import scipy.cluster.hierarchy as shc

plt.figure(figsize=(10, 7))  
plt.title("Pixel dendrograms")  
dend = shc.dendrogram(shc.linkage(cluster_metrics, method='ward'))  

#%% 
from sklearn.cluster import AgglomerativeClustering

cluster = AgglomerativeClustering(n_clusters=120, affinity='euclidean', linkage='ward')  
prediction = cluster.fit_predict(cluster_metrics)  
#%%
cluster_prediction = prediction.reshape(mov_xDim,mov_yDim)
sns.heatmap(cluster_prediction)
