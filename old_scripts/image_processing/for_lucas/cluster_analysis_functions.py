...
from skimage import img_as_ubyte, filters, exposure, io
...
def crop_movie(scaled_mean_image):
    """ Provides an interactive cropping window (rectangle) and returns the cropped 
        area coordinates. Press "R" for confirming the crop.

    Parameters
    ==========
    scaled_mean_image : numpy array
        An image with values scaled between 0-1
        
    Returns
    =======
    x_start : int
        Start pixel of the cropped area in x axis
    x_end : int
        End pixel of the cropped area in x axis
    y_start : int
        Start pixel of the cropped area in y axis
    y_end : int
        End pixel of the cropped area in y axis
...
def get_stim_xml_params(t_series_path, stimInputDir):
    ...
    if not(isRandom):
        ...
        print('\n Baseline epoch selected as 0th epoch\n')
...
def separate_trials(time_series,trialCoor,stimulus_information,
...
def calculate_pixel_max(respTraces_allTrials,stimulus_information,edge_exists):
    ...
    maxEpochIdx_matrix_without_edge = maxEpochIdx_matrix_without_edge + 1 
     ...
     # Finding pixel-wise max epochs
    maxEpochIdx_matrix_all = np.argmax(MaxResp_matrix_all,axis=2) 
    # To assign numbers like epoch numbers
    maxEpochIdx_matrix_all = maxEpochIdx_matrix_all + 1 

    return MaxResp_matrix_all, MaxResp_matrix_without_edge, maxEpochIdx_matrix_without_edge,maxEpochIdx_matrix_all
