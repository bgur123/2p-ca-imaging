...
from helper_functions import plotSaveROISignals
...
def sortStimulusFiles(stimulusPath):
    ...
    all_stim_files_clean.sort(key=lambda \
                        stim_file: os.path.getmtime(os.path.join(stimulusPath,
                                                                 stim_file)))
...
def batchMotionAlignment(rawDataDir, outputDir,granularity ='plane'):
    ...
    for folder_name in all_folders:
        ...
        if 'fly' in folder_name.lower(): 
            ...
            print(">Aligning %s...\n" % (current_exp_ID))
            # Create an ouput experiment directory
            try:
            ...
            for iTseries, t_name in enumerate(t_series_names):
                ...
                # Align T-series
                motionCorrection(t_name, output_path,t_series_files,
                ...
                stimOutFile_path = (glob.glob(stimOutPath))
                if not stimOutFile_path:
                    print('!!Stimulus file not found -> %s!!' % t_series_path)
                else:
                    stimOutFile = stimOutFile_path[0]
                    stim_out_name = os.path.basename(stimOutFile)
                    copyfile(stimOutFile_path[0], os.path.join(output_path,t_name,
                                                       stim_out_name))
                
                # Copying all xml files
                xmlPath = os.path.join(t_series_path, '*.xml')
                xmlFile_path = (glob.glob(xmlPath))
                
                for xml_path_name in xmlFile_path:
