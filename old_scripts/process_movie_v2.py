...
current_exp_ID = '181019_miriam_fly3'
current_t_series = 'TSeries4'
...
SNR_image = cluster_analysis_functions_v2.calculate_pixel_SNR(baselineTraces_allTrials_smooth,
                                                              ...
                                                              SNR_mode='Estimate')
# DSI and CSI 
DSI_image = cluster_analysis_functions_v2.create_DSI_image(stimulus_information, maxEpochIdx_matrix_all_mean,
                                                           max_resp_matrix_all_mean, MeanResp_matrix_all_epochs)
mean_image = time_series.mean(0)
if edge_exists:
    CSI_image = cluster_analysis_functions_v2.create_CSI_image(stimulus_information,
                                                                frameRate, respTraces_allTrials_smooth, DSI_image)
else:
    CSI_image = mean_image
Figures for pixel-maps

cluster_analysis_functions_v2.plot_pixel_maps(mean_image, SNR_image, DSI_image, CSI_image,
                                              current_movie_ID, depth, save_fig=save_fig, save_dir=figure_save_dir)
...
# Extract the data
if stimulus_information['edge_exists']:
    data_to_extract = ['DSI', 'CSI', 'BF', 'SNR', 'reliability', 'uniq_id',
                       'area', 'category', 'CS', 'PD', 'exp_ID', 'stim_name',
                       'edge_response']
else:
...
load_path = os.path.join(saveOutputDir, '190116bg_fly3-TSeries-01162019-1212-003_sima_STICA.pickle')
