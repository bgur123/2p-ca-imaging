...
def plot_roi_masks(roi_image, underlying_image,n_roi1,exp_ID,
    ...
    sns.heatmap(roi_image,alpha=0.5,cmap = 'tab20b',ax=ax1,
                cbar=False)
...
def plot_df_dataset(df, properties, save_name = 'ROI_plots_%s', 
    ...
    fig1, ax1 = plt.subplots(ncols=dim1, nrows=dim2, facecolor='k', edgecolor='w', 
                             figsize=(10, 3))
    axs = ax1.flatten()
    
    for idx, ax in enumerate(axs):
        sns.distplot(df[properties[idx]],ax=ax,color=plt.cm.Dark2(3),kde=False)
...
def run_roi_transfer(transfer_data_path, analysis_type,experiment_info=None,
                     imaging_info=None):
    '''
    '''
    ...
    if analysis_type == 'stripes_OFF_delay_profile':
    ...
    elif analysis_type == 'stripes_ON_delay_profile':
        properties = ['CSI', 'CS','PD','DSI','two_d_edge_profile','category',
                      'extraction_params']
        transferred_rois = ROI_mod.transfer_masks(rois, properties,
                                          experiment_info = experiment_info, 
        ...
        properties = ['CSI', 'CS','PD','DSI','category','RF_maps','RF_map',
                      'RF_center_coords','extraction_params']
        transferred_rois = ROI_mod.transfer_masks(rois, properties,
                                          experiment_info = experiment_info, 
...
def run_analysis(analysis_type, rois,save_fig=True,fig_save_dir = None, 
                 exp_ID=None):
    """
    asd
    """
    if analysis_type == '2D_edges_find_rois_delay_profile_save':
        ...
        rois = ROI_mod.map_RF(rois,edges=True)
        fig1 = ROI_mod.plot_RFs(rois, number=20, f_w =5)
        fig2 = ROI_mod.plot_RF_centers_on_screen(rois,prop='PD')
        fig3 = ROI_mod.plot_RF(rois[len(rois)/2])
        ...
        fig1 = ROI_mod.plot_delay_profile_examples(rois,number=len(rois),f_w=f_w)
        ...
        mean_rsq = map(np.mean,filt_data['resp_delay_fits_Rsq'])
        ...
        ax.plot_joint(plt.scatter, c='w', s=10, linewidth=0.5, marker="o",
                       alpha=.0)
        sns.scatterplot('deg', 'mean_rsq', hue='pd',data=df,alpha=.8,
                        palette=[plt.cm.Dark2(0), plt.cm.Dark2(2)])
        ...
        fig2 = ROI_mod.plot_RF_centers_on_screen(rois,prop='BF',
                                                 ...
                                                 lims=(0,1.5))
        fig3 = ROI_mod.plot_RF(rois[len(rois)/2])
        fig4 = ROI_mod.plot_RF_centers_on_screen(rois,prop='PD')
        ...
        if save_fig:
           ...
           fig3.savefig('%s.png'% f3_n, bbox_inches='tight',
                       transparent=False,dpi=300)
           fig4.savefig('%s.png'% f4_n, bbox_inches='tight',
                       transparent=False,dpi=300)
